<template>
    <form v-on:submit.prevent="validateAndSubmit" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group" v-bind:class="{ 'has-error' : errors.has('description') || errors.description}">
            ...
            <textarea id="txt_desc" name="description" v-model="event.description" v-validate="'required|min:100'" class="form-control" ></textarea>
        ...
        <div class="form-group" v-bind:class="{'has-error': errors.has('contract') || errors.contract }">
            <file-upload-input label="Contract with artist" name="contract" type="file"></file-upload-input>
            <form-error v-if="errors.has('contract') || errors.contract" :errors="errors">
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.$validator.attach('contract', 'required');
            this.$validator.attach('location', 'required');
            this.$validator.attach('lat', 'required');
            this.$validator.attach('lng', 'required');
            this.$validator.attach('start_date', "required|after:"+moment().format('YYYY-MM-DD HH:mm:ss')+"|date_format:YYYY-MM-DD HH:mm");
            this.$validator.attach('end_date', "nullable|after:event.start_date|date_format:YYYY-MM-DD HH:mm");
        ...
        methods: {
            submit() {
                ...
                var url =url = Helpers.auth_url('/api/events/create-application', App.User)  
